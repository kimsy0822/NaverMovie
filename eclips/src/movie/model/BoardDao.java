...
public class BoardDao extends DBManager {
	...
	//아이디별 리뷰리스트
	public ArrayList<BoardVo> boardSelectById(String id,PagingVo pagingVO){
	...
	//아이디별 검색결과수
	public int reviewCountById(String id,PagingVo pagingVO) { //변수를 void=>int로 변경 게시물조회 복사 후 수정
	...
	//리뷰가 많은 영화 정렬 리스트
	public ArrayList<BoardVo> topmovie(){
		
		ArrayList<BoardVo> rst = new ArrayList<BoardVo>();
		conn = getConnect();
		String sql = "select m.mid,m.mname,avg(b.score) as avgscore, count(b.mid) from board as b left join movie m on b.mid = m.mid group by b.mid order by count(b.mid) desc, avgscore desc";
		try{
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()){
				...
				Btemp.setStr_avgscore(String.format("%.2f",rs.getDouble("avgscore")));
	...
	//영화아이디별 최근 리뷰글
	public ArrayList<BoardVo> selectreview(String mid) {
		
		ArrayList<BoardVo> rst = new ArrayList<BoardVo>();
		
		conn = getConnect();
		
		String sql = "select * from board where mid = ? order by reg_date desc";
		try{
	...
	public ArrayList<BoardVo> topmoviereview(){
		
		ArrayList<BoardVo> rst = new ArrayList<BoardVo>();
		
		conn = getConnect();
		
		String sql ="select r.*,x.* from (select b.*,m.mname from board as b left join movie as m on b.mid=m.mid) as r right join (select b.mid,avg(score) as avgscore,count(b.mid) as rcount from board as b group by b.mid) as x on r.mid=x.mid order by rcount desc, r.mname, reg_date desc";
	...
	public int deleteReviewall(String id) {
